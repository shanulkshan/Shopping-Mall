import {
  ...
  export default function UpdatebookShop() {
      ...
      try {
        const fetchForm = async () => {
          const res = await fetch(
            `/api/cloth/CgetAll?UupdateId=${updateCId}`
          );
          ...
          if (res.ok) {
            const selectedform = data.getcloth.find((formm) => formm._id === updateCId);
            if (selectedform) {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        const res = await fetch(`/api/cloth/cloth/${formData._id}`, {
          method: "PUT",
          headers: {
    ...
    return (
        ...
        <div className="">
        <h1 className="text-center text-3xl my-7 ml-10 font-medium">Update Shop</h1>
